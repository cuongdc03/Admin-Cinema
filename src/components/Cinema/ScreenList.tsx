...
const ScreenList: React.FC<ScreenListProps> = ({ cinemaId, onEdit }) => {
  ...
  useEffect(() => {
    const fetchScreens = async () => {
      try {
        const response = await fetch(`https://bl924snd-3000.asse.devtunnels.ms/admin/cinema/${cinemaId}`); 
        if (!response.ok) {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'totalSeats', headerName: 'Total Seats', width: 200, resizable: false }, 
    {
      field: 'actions',
      headerName: 'Actions',
      resizable: false,
