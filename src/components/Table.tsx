...
import { dataGridSettings } from './constants'
...
const Table: React.FC<TableProps<any>> = ({ rows, onDelete, onStatusChange, displayedColumns, isCinema }) => {
  ...
  const theme = createTheme({
    palette: {
      primary: {
        main: dataGridSettings.theme.primaryMain,
        contrastText: dataGridSettings.theme.primaryContrastText
      },
      mode: dataGridSettings.theme.lightMode
    },
    components: {
      MuiDataGrid: {
        styleOverrides: {
          columnHeaderTitleContainer: ({ theme }) => ({
            ...
            '&.MuiDataGrid-columnHeaderTitleContainer--withBackground.dark': {
              color: dataGridSettings.theme.columnHeaderTitleContainer.withBackground
            }
          })
        }
      }
  ...
  const columns: GridColDef[] = displayedColumns.map((key) => ({
    ...
    resizable: false,
    width: displayedColumns.length === 4 ? dataGridSettings.columnWidth.extended : dataGridSettings.columnWidth.default,
    renderCell: (params) =>
      key === 'status' ? (
        <Switch checked={params.row.status} onChange={() => handleStatusChange(params.row)} />
      ) : (
        params.value
      )
  }))
      ...
      <ThemeProvider theme={theme}>
        <div style={{ height: 600, width: '100%' }}>
          <DataGrid
            ...
            rows={filteredRows}
            columns={columns}
            pageSizeOptions={dataGridSettings.pageSizeOptions}
            disableRowSelectionOnClick={dataGridSettings.disableRowSelectionOnClick}
            autoHeight={dataGridSettings.autoHeight}
            disableColumnFilter={dataGridSettings.disableColumnFilter}
            disableColumnSelector={dataGridSettings.disableColumnSelector}
          />
        </div>
      </ThemeProvider>
      <div className={`fixed inset-0 z-50 flex items-center justify-center ${showModal ? 'block' : 'hidden'}`}>
        <div className='w-96 rounded-lg bg-white p-6 shadow-lg' style={{ width: dataGridSettings.modal.width }}>
