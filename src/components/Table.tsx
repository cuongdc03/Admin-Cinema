...
const Table = <T extends { id: number; status?: boolean; [key: string]: any }>({
  ...
  const columns: GridColDef[] = displayedColumns.map((key) => ({
    ...
    renderCell: (params) => {
      if (key === 'status') {
        return <Switch checked={params.row.status as boolean} onChange={() => handleStatusChange(params.row)} disabled />
  ...
  columns.push({
    ...
    renderCell: (params) => (
      <div className='flex h-full w-full items-center justify-center gap-4'>
        <Link to={`${params.row.id}`} className='flex rounded bg-blue-500 px-4 py-2 font-bold text-white hover:bg-blue-700'>
          <MdEdit />
        </Link>
        {isCinema ? (
          <button onClick={() => handleArchiveClick(params.row)} className='mr-2 rounded bg-yellow-500 px-4 py-2 font-bold text-white hover:bg-yellow-700'>
            <FaArchive />
          </button>
        ) : (
          <button onClick={() => handleDelete(params.row)} className='mr-2 rounded bg-red-500 px-4 py-2 font-bold text-white hover:bg-red-700'>
            <MdDelete />
          </button>
        )}
      </div>
    ...
    <div className='flex flex-col'>
      <div className='mb-4 flex items-center justify-between'>
        ...
        <div className='flex items-center'>
          <Link to='create' className='relative mr-2 items-center justify-center rounded-md border border-primary px-10 py-2 text-center font-medium text-primary hover:bg-opacity-90'>
            Create
          </Link>
        </div>
      </div>
      ...
      <div className={`fixed inset-0 z-50 flex items-center justify-center ${showModal ? 'block' : 'hidden'}`}>
        <div className='w-96 rounded-lg bg-white p-6 shadow-lg'>
          ...
          <p className='text-gray-600 mb-4'>Are you sure you want to {isCinema ? 'archive' : 'delete'} this {isCinema ? 'cinema' : 'item'}?</p>
          <div className='flex justify-between'>
            <button onClick={handleCancelDelete} className='hover:bg-gray-500 rounded bg-blue-400 px-4 py-2 font-bold text-white'>
              Cancel
            </button>
            <button onClick={isCinema ? handleConfirmArchive : handleConfirmDelete} className='rounded bg-red-500 px-4 py-2 font-bold text-white hover:bg-red-700'>
